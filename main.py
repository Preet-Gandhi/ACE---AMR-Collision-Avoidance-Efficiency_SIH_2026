...
def build_simulation(config=None):
    ...
    robots = [Robot(i, (i, 0), warehouse, planner, network, reservations, config.initial_battery, config.robot_speed, config.congestion_penalty, config.priority_bonus, config.invalid_bid_penalty, config.orca_enabled, config.orca_neighbor_distance, config.orca_time_horizon, config.orca_robot_radius, config.orca_max_speed, config.distributed_auction, config.reservation_lease) for i in range(config.num_robots)]
    ...
    for task in warehouse.get_pending_tasks():
        if config.distributed_auction:
            auction.start_distributed(task)
        else:
            auction.run_auction(task)
    return Simulator(warehouse, robots, network, reservations, metrics, auction, config.simulation_dt, config.orca_enabled), robots
